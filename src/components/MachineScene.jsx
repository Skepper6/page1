...
export default function MachineScene({ onReady }) {
  const host = useRef(null); useEffect(() => {
    ...
    const updateModel=model.update;model.update=()=>{updateModel();model.machine.scale.x*=.88;model.machine.scale.z*=.9;model.machine.updateMatrixWorld(true);camera.updateMatrixWorld(true);window.__orbisAnchorPoints=model.anchors.map(o=>{const p=o.getWorldPosition(new THREE.Vector3()).project(camera);return [(p.x+1)*640,(1-p.y)*360];});window.dispatchEvent(new Event('orbis-anchors'));};
    let measuredWidth=0,measuredHeight=0;
    const measure = () => { const w = host.current.clientWidth, h = host.current.clientHeight;if(w===measuredWidth&&h===measuredHeight)return;measuredWidth=w;measuredHeight=h; renderer.setSize(w, h, false); camera.aspect = w / h; camera.updateProjectionMatrix();requestAnimationFrame(()=>window.__orbisModel?.render(true)); }; measure(); const observer = new ResizeObserver(measure); observer.observe(host.current);
