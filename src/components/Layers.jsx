...
export function Copy({go}){
 useEffect(()=>{
  ...
  const align=()=>{if(!active)return;const bounds=box.getBoundingClientRect();if(!bounds.width||!bounds.height)return;const paths=box.querySelectorAll('.leaders path');const dots=box.querySelectorAll('.leaders circle');const targets=[[640,352],[640,401],[650,401],[650,568]];
   box.querySelectorAll('.label').forEach((label,i)=>{const r=label.getBoundingClientRect();const left=(r.left-bounds.left)/bounds.width*1280;const right=(r.right-bounds.left)/bounds.width*1280;const y=(r.bottom-bounds.top+10)/bounds.height*720;const [x,endY]=targets[i];const start=i<2?left:right;const elbow=i<2?Math.min(right+30,x-70):left;paths[i].setAttribute('d',`M${start} ${y}H${elbow}L${x} ${endY}`);dots[i].setAttribute('cx',x);dots[i].setAttribute('cy',endY);});
  };const observer=new ResizeObserver(align);observer.observe(box);box.querySelectorAll('.label').forEach(e=>observer.observe(e));document.fonts.ready.then(align);align();return()=>{active=false;observer.disconnect();};
 },[]);
 return <>
 ...
 <section className="precision copy"><h2>WATCH PRECISION<br/>COME APART</h2></section>
 ...
 <section className="copy bean-story"><span className="number">3</span><h2>FROM BEAN<br/>TO SOMETHING<br/>BETTER</h2><p>ORBIS adapts the ritual around you—not<br/>the other way around.</p><button className="pill dark" onClick={()=>go(6.65)}>FOLLOW THE BEAN <span>↗</span></button></section>
